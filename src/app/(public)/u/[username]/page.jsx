            ...
            <div className="grid grid-cols-3 gap-1.5 sm:gap-4">
              {posts.map((post) => (
                <Link
                  key={post.id}
                  href={`/p/${post.id}`}
                  className="group relative aspect-square bg-[var(--surface)] border border-[var(--line)] overflow-hidden rounded-lg focus-visible:outline-2 focus-visible:outline-[var(--accent-bright)]"
                >
                  {post.imageUrl ? (
                    <Image
                      src={post.imageUrl}
